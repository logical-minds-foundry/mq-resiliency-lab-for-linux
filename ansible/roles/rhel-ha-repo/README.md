...
9, Rocky 9, or CentOS Stream 9 — into `build/state/rhel-ha/` (gitignored). E.g. on an
EL9-compatible host (or container) with the HighAvailability repo enabled:

    dnf download --resolve --downloaddir=build/state/rhel-ha \
...
instead, populate `build/state/rhel-ha/` from the entitled repo — the role is
unchanged.
