...
| `work/`  | local  | **every** `mqlab build clean`               | deterministic renders — `inventory.ini`, `lab/topology.resolved.yaml`, `box-versions.json`, `versions.json`, `grafana/`, `prometheus/`, `obs/`, `logsearch/` (the Alloy→OpenSearch fan-out gate `fanout.json`, written by `mqlab commons up`), `salt/`, manifest overlays |
