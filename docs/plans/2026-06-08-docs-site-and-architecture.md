...
> **Build-recipe correction (from the Task 1 spike).** The original recipe
> in this plan (`vrg-container-run -- uv run mkdocs …`) does not work:
> mkdocs/mike are not in the project venv, and `vrg-container-run` syncs only
> the `dev` group. The sanctioned local tool is **`vrg-container-docs`**,
> which runs `uv sync --group docs && uv run mkdocs …` inside a docs-capable
> container. It requires a `[dependency-groups] docs` table (added in Task
> 1). It does **not** stage the changelog/release sources, so stage them once
> (Task 1) into gitignored paths; the staged files persist in the working
> tree for repeated builds.
...
```bash
# Once per session (or after CHANGELOG.md / releases/ change): stage the
# changelog + release-notes sources into the docs tree (outputs gitignored).
vrg-container-run -- vrg-docs-stage --docs-dir docs/site/docs

# Strict build — the red/green signal. Mirrors the CI/CD build.
vrg-container-docs build --strict

# Optional live preview while authoring diagrams/pages:
vrg-container-docs serve        # http://localhost:8000
```
...
**Committed vs generated.** The changelog/release-notes *sources*
(`CHANGELOG.md`, `releases/`) are committed at the repo root. Their *staged
copies* (`docs/site/docs/changelog.md`, `docs/site/docs/releases/`) and the
build output (`docs/site/site/`) are generated and gitignored (Task 1).
`vrg-docs-patch-nav` injects per-version nav entries and is a CI concern;
pre-release it is a no-op, so it is omitted from the local loop.

**Nav always resolves.** Task 1 creates stub pages for every nav target
(`architecture/index.md`, `design-and-specs.md`), so the strict build passes
after every task; later tasks replace stubs with real content. Ignore any
"temporarily comment the nav line" notes in later tasks — they are obsolete
under the stub-first approach.
...
| Path | Responsibility |
|------|----------------|
| `pyproject.toml` | Adds the `[dependency-groups] docs` table (mkdocs-material, mike) that `vrg-container-docs` and CI sync. |
| `uv.lock` | Regenerated to include the docs group. |
| `CHANGELOG.md` | **Committed source**, generated by `vrg-changelog --changelog-only` from commit history; staged into the site. |
| `releases/.gitkeep` | **Committed source** dir for per-version release notes (empty until the first tagged release). |
...
| `.gitignore` | Ignore *staged/generated* docs artifacts (`docs/site/docs/changelog.md`, `docs/site/docs/releases/`, `docs/site/site/`). |
...
build that matches CI, plus the dormant-but-wired Releases machinery. We
establish the toolchain, the committed changelog/release sources, the
gitignored staged outputs, and stub pages for every nav target — leaving the
site green and template-aligned. Spike-first, fail-loud.

**Files:**
- Modify: `pyproject.toml`, `uv.lock` (docs dependency group)
- Create: `CHANGELOG.md` (committed source), `releases/.gitkeep` (committed source)
- Modify: `docs/site/mkdocs.yml`, `.gitignore`
- Create: stub `docs/site/docs/architecture/index.md`, `docs/site/docs/design-and-specs.md`
...
- [ ] **Step 2: Generate the committed changelog source + empty releases source**

```bash
vrg-container-run -- vrg-changelog --changelog-only   # writes ./CHANGELOG.md from commit history
mkdir -p releases && : > releases/.gitkeep            # committed release-notes source dir (empty pre-release)
```

Confirm `CHANGELOG.md` exists and contains the conventional-commit history
(an "unreleased" section, since there are no tags yet). If `vrg-changelog`
fails or produces nothing, STOP and report — do not hand-write a fake
changelog.

- [ ] **Step 3: Add staged-artifact ignores to `.gitignore`**

Append to `.gitignore` (after the "Test/coverage artifacts" block). Note we
ignore the *staged copies* under `docs/site/docs/`, never the committed
`CHANGELOG.md`/`releases/` sources at the repo root:

```gitignore
# Staged/generated docs artifacts — vrg-docs-stage copies the root
# CHANGELOG.md/releases sources here and mkdocs builds into site/. The
# sources are committed; these copies are regenerated each build.
docs/site/site/
...
- [ ] **Step 5: Create stub pages for the not-yet-written nav targets**

So the strict build passes from here on (later tasks replace these stubs):

```bash
mkdir -p docs/site/docs/architecture
printf '# Architecture\n\n_Stub — content lands in Tasks 5–10._\n' > docs/site/docs/architecture/index.md
printf '# Design & Specs\n\n_Stub — content lands in Task 11._\n' > docs/site/docs/design-and-specs.md
```

(`index.md` and `getting-started.md` already exist as stubs.)

- [ ] **Step 6: Stage sources + strict build → green baseline**
...
Expected: `Documentation built in …` with no warnings. The build now
resolves every nav entry: Home, Releases (Changelog from the staged
`CHANGELOG.md`; Release Notes index placeholder), Getting Started,
Architecture (stub), Design & Specs (stub). If it fails, fix before
committing — this is the harness everything else relies on.

- [ ] **Step 7: Commit**

The staged `docs/site/docs/changelog.md`, `docs/site/docs/releases/`, and
`docs/site/site/` are gitignored, so they will not be staged for commit.

```bash
vrg-git add pyproject.toml uv.lock CHANGELOG.md releases/.gitkeep \
  docs/site/mkdocs.yml .gitignore \
  docs/site/docs/architecture/index.md docs/site/docs/design-and-specs.md
vrg-commit --type docs --scope docs --message "wire docs site: docs deps, dormant Releases machinery, template nav, stubs"
```
