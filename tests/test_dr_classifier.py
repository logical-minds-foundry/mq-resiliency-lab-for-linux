...
def test_continued_when_replicated_to_secondary():
    assert (
        classify(_facts(on_secondary=True, svc_received=1, svc_replied=True)) is Bucket.CONTINUED
    )
...
def test_precedence_confirmed_beats_continued():
    assert (
        classify(_facts(app_confirmed=True, on_secondary=True, svc_received=1))
        is Bucket.CONFIRMED
    )
