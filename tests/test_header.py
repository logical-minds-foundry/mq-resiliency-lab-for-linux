...
def test_roundtrip():
    ...
    assert (f.sender, f.receiver, f.session_date, f.payload) == (
        "APP01",
        "SVC",
        "20260606",
        "PAYLOAD",
...
def test_pack_header_rejects_overlong_field():
    with pytest.raises(ValueError, match="sender exceeds"):
        pack_header(password="pw", sender="TOOLONGAPP", receiver="SVC", session_date="20260606")


def test_validate_rejects_blank_required_fields():
    blank_sender = pack_header(password="pw", sender="", receiver="SVC", session_date="20260606")
