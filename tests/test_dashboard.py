...
def test_mq_service_rows_use_confirmed_ibmmq_metrics():
    ...
    status = by_title["PCMKAPP — status"]["targets"][0]["expr"]
    assert status == 'max(ibmmq_qmgr_status{qmgr="PCMKAPP"}) or vector(-1)'
    assert "ibmmq_qmgr_connection_count" in by_title["PCMKAPP — connections"]["targets"][0]["expr"]
    rate = by_title["PCMKAPP — msg rate"]["targets"][0]["expr"]
...
def test_status_tiles_map_to_coloured_strings():
    ...
    texts = {
        opt["text"]
        for m in by_title["PCMKAPP — status"]["fieldConfig"]["defaults"]["mappings"]
        for opt in m["options"].values()
    }
    assert {"No status", "Stopped", "Transitioning", "Running"} == texts
...
def test_queue_table_is_name_plus_depth_only():
    by_title = {p.get("title"): p for p in render_dashboard(TOPO)["panels"]}
    organize = by_title["PCMKAPP — queues"]["transformations"][0]["options"]
...
def test_queue_rate_graphs_show_enqueue_and_dequeue():
    by_title = {p.get("title"): p for p in render_dashboard(TOPO)["panels"]}
    exprs = [t["expr"] for t in by_title["PCMKAPP — queue rates"]["targets"]]
