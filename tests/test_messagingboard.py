...
def test_board_is_per_stack_parameterized_with_no_cross_stack_leak():
    ...
    assert "PCMKAPP" not in json.dumps(
        render_messaging_board("rdqm-rhel", "RDQM", "RDQMAPP", "SVCQM", "RDQM.SVC.REQUEST")
    )
...
def test_flow_strip_depth_tile_uses_the_per_stack_request_queue():
    ...
    exprs = _exprs(
        render_messaging_board("pcmk-ubuntu", "PCMK", "PCMKAPP", "SVCQM", "PCMK.SVC.REQUEST")
    )
...
def test_status_band_is_flow_indicators_not_depth_sum():
    ...
    blob = json.dumps(render_messaging_board("s", "S", "APP", "SVC", "S.SVC.REQUEST"))
...
def test_status_band_app_qm_tile_drills_into_the_per_qm_board():
    # #489: the App QM status tile links to that stack's per-QM state board.
    d = render_messaging_board("s", "S", "APP", "SVC", "S.SVC.REQUEST")
    app_qm_tile = next(
        p for p in d["panels"] if p.get("type") == "stat" and p.get("title") == "App QM"
    )
    links = app_qm_tile["fieldConfig"]["defaults"]["links"]
    assert links == [{"title": "QM state ↗", "url": "/d/lab-qm-s"}]
