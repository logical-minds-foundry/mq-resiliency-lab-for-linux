...
def test_board_is_per_stack_parameterized_with_no_cross_stack_leak():
    d = render_messaging_board("pcmk-ubuntu", "PCMKAPP", "PCMKSVC")
    blob = json.dumps(d)
    assert d["uid"] == "lab-messaging-pcmk-ubuntu"
    assert "messaging" in d["tags"] and "pcmk-ubuntu" in d["tags"]
    assert "PCMKAPP" in blob and "PCMKSVC" in blob
    assert "PCMKAPP.PCMKSVC" in blob  # the flow-strip SDR hop is <app>.<svc>
    # a different stack's board never carries this stack's QMs
    assert "PCMKAPP" not in json.dumps(render_messaging_board("rdqm-rhel", "RDQMAPP", "RDQMSVC"))


def test_status_band_is_flow_indicators_not_depth_sum():
    exprs = _exprs(render_messaging_board("s", "APP", "SVC"))
